...
if (btn_exit) {
  ...
  const in3 = document.getElementById("email");
  const in4 = document.getElementById("pass");
  ...
  function authForm() {
    ...
    if (!/^[A-Za-z]+@[a-z]+[.][a-z]+$/.test(in3.value) && in3.value != "") {
      in3.setCustomValidity("Некорректный email.");
    }
    else {
      in3.setCustomValidity("");
    }

    if (in4.value.length >= 20) {
      in4.setCustomValidity("Длина пароля не может быть больше 20 символов");
    }
    else {
      in4.setCustomValidity("");
    }
  }
}
